...
- **Practice sets** are graded by the TA on Gradescope. Every question is checked against the posted solution key; two or three questions per set additionally receive written feedback. See [Practice Set Policies](practice-sets.qmd) for why the depth varies.
- **Quizzes** are graded by Prof. Srikrishnan and the TA using the same principles as the practice set key. These are scanned and graded on Gradescope.
...
- **Practice sets can be submitted up to 24 hours late for full credit.** The solution key is posted when that window closes, and no submissions are accepted after that point. The lowest practice set is dropped.
...
One practice set, one quiz, and one reading are dropped automatically. That is not slack for you to plan around --- it is insurance for the week when something goes wrong. If you burn all three in February because you were busy, and then get sick in April, we are both in a worse position than if you had come to talk to me.
:::
